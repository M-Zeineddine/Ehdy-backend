...
**STATUS:** MIGRATION DRAFTED, **renumbered → `src/migrations/drafts/018_gift_draft_id_idempotency.sql`** (A8 is void; the new 017 is the is_featured NOT NULL reconciliation). Awaiting approval; not run. Amended 2026-07-09: FK is `ON DELETE SET NULL` (retry drafts are deleted on payment success); a stale-pending sweeper cron is scoped into the same PR (30-min window, skips charges with a CAPTURED webhook) — query pending approval, not yet implemented.
...
**STATUS:** REPORTED — not implemented.
